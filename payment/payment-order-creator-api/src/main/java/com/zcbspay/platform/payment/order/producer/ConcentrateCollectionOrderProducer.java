...
public class ConcentrateCollectionOrderProducer implements Producer{
	private final static Logger logger = LoggerFactory.getLogger(ConcentrateCollectionOrderProducer.class);
	...
	public ConcentrateCollectionOrderProducer(String namesrvAddr) throws MQClientException{
		logger.info("【初始化ConcentrateOrderProducer】");
        ...
        topic = RESOURCE.getString("concentrate.collection.order.subscribe");
        logger.info("【初始化ConcentrateOrderProducer结束】");
	}
	...
	public SendResult sendJsonMessage(String message, OrderTagsEnum tags)
			...
			MQBrokerException {
		if(producer==null){
			throw new MQClientException(-1,"ConcentrateCollectionOrderProducer为空");
		}
		producer.start();
